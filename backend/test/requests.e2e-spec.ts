...
describe('RequestsController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    requestsRepository = moduleFixture.get<Repository<RequestEntity>>(
      getRepositoryToken(RequestEntity),
    );
    skillsRepository = moduleFixture.get<Repository<SkillEntity>>(
    ...
    await requestsRepository.query(
      'TRUNCATE TABLE requests, skills, users CASCADE;',
    );
  ...
  describe('PATCH /requests/:id/accept', () => {
    ...
    it('should accept request and exchange skills', async () => {
      ...
      const senderSkillsBefore = await skillsRepository.count({
        where: { owner: { id: senderId } },
      });
      const receiverSkillsBefore = await skillsRepository.count({
        where: { owner: { id: receiverId } },
      });
      ...
      const senderSkillsAfter = await skillsRepository.count({
        where: { owner: { id: senderId } },
      });
      const receiverSkillsAfter = await skillsRepository.count({
        where: { owner: { id: receiverId } },
      });
