...
let AuthService = class AuthService {
    ...
    jwtConfiguration;
    usersService;
    constructor(usersRepository, jwtService, jwtConfiguration, usersService) {
    ...
    async refresh(refreshTokenDto) {
        let payload;
        try {
            payload = await this.jwtService.verifyAsync(refreshTokenDto.refreshToken, { secret: this.jwtConfiguration.refreshSecret });
        }
        catch {
            throw new common_1.UnauthorizedException('Невалидный или истёкший refresh токен');
        }
        const user = await this.usersRepository.findOne({
    ...
    async logout(userData) {
        ...
        const isRefreshTokenValid = await bcrypt.compare(userData.refreshToken, user.refreshToken);
        if (!isRefreshTokenValid) {
            throw new common_1.UnauthorizedException('Невалидный или истёкший refresh токен');
        }
...
exports.AuthService = AuthService = __decorate([
    ...
    __metadata("design:paramtypes", [typeorm_2.Repository,
        jwt_1.JwtService, Object, users_service_1.UsersService])
], AuthService);
